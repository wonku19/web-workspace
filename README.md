...
5. 컨테이너에 서블릿에 대한 설정은 배포 서술자(web.xml)에 작성한다

### 배포 서술자(Deployment Descroptor)

- 배포 서술자 (Deployment Descroptor) 는 애플리케이션에 대한 전체 설정 정보를 가지고 있는 파일
...
- getWriter() : 문자를 페이지에 전송하기 위한 Stream 을 가져오는 메소드
- setContentType(String) : 응답으로 작성하는 페이지의 MINE type 을 정하는 메소드
