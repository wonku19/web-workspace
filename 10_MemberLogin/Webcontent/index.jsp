	...
	<ul>
		...
		<%-- 
			회원가입 : 아이디, 비밀번호, 이름, 주소 입력 받아서 
					-> RegisterServlet / post 방식으로 전달 
		         	-> session에 데이터 바인딩 
		         	-> session 에 값이 있는 경우 = AllMemberServlet
		         	-> session 에 값이 없는 경우 = index.jsp 
		--%>
		
		<li><a href="views/login.html">로그인</a></li>
		<%-- 
			로그인 -> 아이디, 비밀번호 입력해서
			-> LoginServley / post 방식
			-> session에 데이터 바인딩 -> views/login_result.jsp (정보 출력)
		--%>
